...
unsigned long gyroPreviousMillis = 0;       // will store last time sensor updated

// the follow variables is a long because the time, measured in miliseconds,
// will quickly become a bigger number than can be stored in an int.
const long interval = 10000;            // Sensor data sending interval
const long gyroInterval = 1000;  // Accelerator and Gyroscope interval
...
int16_t store_ax, store_ay, store_az;
int16_t store_gx, store_gy, store_gz;
int16_t comp_ax, comp_ay, comp_az;
int16_t comp_gx, comp_gy, comp_gz;
...
void makeSounds()
{
  startPlayback2(sample, sizeof(sample));
}
...
void adjustTrack()
{
  accelgyro.getMotion6(&store_ax, &store_ay, &store_az, &store_gx, &store_gy, &store_gz);
  // Vertaa saatuja lukuja edellisiin ja tee korjaavat liikkeet moottoreilla
...
void driveMotors()
{
  ...
  // the PWM values sent by analogWrite() are fractions of the maximum speed possible by your hardware
    ...
    if (course == 1) {
      ...
      analogWrite(enA, 1.06*mspeed); analogWrite(enB, mspeed );
    }
    // Drive Backward
    else if (course == 2 ) {
...
void HandleIncomingJson() {
  ...
  if (commandAsString =="drive") {motor_active = true; mStartMillis = millis(); motor_always_on = false;}
  if (commandAsString =="drivealways") {motor_active = true; motor_always_on = true;}
  if (commandAsString =="stop") {motor_always_on = false; }
  }
...
void setup() {
  ...
  // Egde sensor input pins   -- It does not exist for us!
  ...
  dht.begin(); // Init DHT  -- Humidity and temperature sensor
...
/*
  ...
  accelgyro.setZAccelOffset(az); // 1688 factory default for my test chip
*/
...
void loop() {
  ...
  // Distance reporting when motors not running
  ...
  // AccGyroReporting when motors not running
  if (currentMillis - gyroPreviousMillis >= gyroInterval) {
    // save the last time of sensor reporting
    readTime();
    gyroPreviousMillis = currentMillis;    
    if (motor_active == false) {
     JsonReportSensorAccAndGyro();
    } // slow down reporting frequency in static cases
  }
  
  // Sensor reporting when motors are RUNNING
  if (motor_active == true) { // speed up reporting frequency in case of moving
    ...
    if (currentMillis - mStartMillis >= sensor_moving_reports ) {
      readTime();
      JsonReportSensorDistance();
      //JsonReportSensorEdge();   -- not existing
      JsonReportSensorAccAndGyro();
    }
  }
